...
export const BattlePage = () => {
  ...
  useEffect(() => {
    const fetchAIPrediction = async () => {
      try {
        const res = await axios.post(`${BACKEND}/api/mock_ai/predict`, {
          marketId: 1,
          aiId: selectedAIModel
        });
        if (res.data.ok) {
          const pred = JSON.parse(res.data.prediction.prediction);
          setConfidence(res.data.prediction.confidence * 100);
          setTrend(pred.up > pred.down ? 'Bullish' : 'Bearish');
        }
      } catch (error) {
